...
const lanyard = ({ products }) => {
    return (
        <div className=" flex flex-col items-center">
            <h1 className="text-3xl mb-8">Transfer termic pe landyarduri</h1>
...
export async function getServerSideProps() {
    ...
    const lanyardCategory = await Category.findOne({ name: "Transfer termic pe landyarduri" });

    // If the category is found, fetch products with that category
    const products = lanyardCategory
        ? await Product.find({ category: lanyardCategory._id }, null, { sort: { '_id': -1 } })
        : [];
