...
const chipiu = ({ products }) => {
    return (
        <div className=" flex flex-col items-center">
            <h1 className="text-3xl mb-8">Chipiuri</h1>
...
export async function getServerSideProps() {
    ...
    const chipiuCategory = await Category.findOne({ name: "Chipiuri" });

    // If the category is found, fetch products with that category
    const products = chipiuCategory
        ? await Product.find({ category: chipiuCategory._id }, null, { sort: { '_id': -1 } })
        : [];
