...
const hanorac = ({ products }) => {
  return (
    <div className=" flex flex-col items-center">
      <h1 className="text-3xl mb-8">Hanorace</h1>
...
export default hanorac;

export async function getServerSideProps() {
  await mongooseConnect();

  // Find the category with the name "Pixuri Eco"
  const hanoracCategory = await Category.findOne({ name: "Hanorace" });

  // If the category is found, fetch products with that category
  const products = hanoracCategory
    ? await Product.find({ category: hanoracCategory._id }, null, { sort: { '_id': -1 } })
    : [];
