...
const semne = ({ products }) => {
    return (
        <div className=" flex flex-col items-center">
            <h1 className="text-3xl mb-8">Semne de carte</h1>
...
export async function getServerSideProps() {
    ...
    const semneCategory = await Category.findOne({ name: "Semne de carte" });

    // If the category is found, fetch products with that category
    const products = semneCategory
        ? await Product.find({ category: semneCategory._id }, null, { sort: { '_id': -1 } })
        : [];
