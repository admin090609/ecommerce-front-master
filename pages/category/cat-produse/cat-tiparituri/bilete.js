...
const bilete = ({ products }) => {
    return (
        <div className=" flex flex-col items-center">
            <h1 className="text-3xl mb-8">Bilete</h1>
...
export async function getServerSideProps() {
    ...
    const bileteCategory = await Category.findOne({ name: "Bilete" });

    // If the category is found, fetch products with that category
    const products = bileteCategory
        ? await Product.find({ category: bileteCategory._id }, null, { sort: { '_id': -1 } })
        : [];
