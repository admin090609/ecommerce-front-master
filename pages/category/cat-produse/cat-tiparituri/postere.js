...
const postere = ({ products }) => {
    return (
        <div className=" flex flex-col items-center">
            <h1 className="text-3xl mb-8">Postere</h1>
...
export async function getServerSideProps() {
    ...
    const postereCategory = await Category.findOne({ name: "Postere" });

    // If the category is found, fetch products with that category
    const products = postereCategory
        ? await Product.find({ category: postereCategory._id }, null, { sort: { '_id': -1 } })
        : [];
