...
const mape = ({ products }) => {
    return (
        <div className=" flex flex-col items-center">
            <h1 className="text-3xl mb-8">Mape de prezentare</h1>
...
export async function getServerSideProps() {
    ...
    const mapeCategory = await Category.findOne({ name: "Mape de prezentare" });

    // If the category is found, fetch products with that category
    const products = mapeCategory
        ? await Product.find({ category: mapeCategory._id }, null, { sort: { '_id': -1 } })
        : [];
