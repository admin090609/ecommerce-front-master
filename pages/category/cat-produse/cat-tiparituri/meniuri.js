...
const meniuri = ({ products }) => {
    return (
        <div className=" flex flex-col items-center">
            <h1 className="text-3xl mb-8">Meniuri</h1>
...
export async function getServerSideProps() {
    ...
    const meniuriCategory = await Category.findOne({ name: "Meniuri" });

    // If the category is found, fetch products with that category
    const products = meniuriCategory
        ? await Product.find({ category: meniuriCategory._id }, null, { sort: { '_id': -1 } })
        : [];
