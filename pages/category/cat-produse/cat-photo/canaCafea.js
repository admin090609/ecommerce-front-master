...
const canaCafea = ({ products }) => {
    return (
        <div className=" flex flex-col items-center">
            <h1 className="text-3xl mb-8">Cană pentru cafea</h1>
...
export default canaCafea;

export async function getServerSideProps() {
    await mongooseConnect();

    // Find the category with the name "Pixuri Eco"
    const canaCafeaCategory = await Category.findOne({ name: "Cană pentru cafea" });

    // If the category is found, fetch products with that category
    const products = canaCafeaCategory
        ? await Product.find({ category: canaCafeaCategory._id }, null, { sort: { '_id': -1 } })
        : [];
