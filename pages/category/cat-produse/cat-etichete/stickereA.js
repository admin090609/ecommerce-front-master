...
const stickereA = ({ products }) => {
    return (
        <div className=" flex flex-col items-center">
            <h1 className="text-3xl mb-8">Stickere A-size</h1>
...
export default stickereA;

export async function getServerSideProps() {
    await mongooseConnect();

    // Find the category with the name "Pixuri Eco"
    const stickereACategory = await Category.findOne({ name: "Stickere A-size" });

    // If the category is found, fetch products with that category
    const products = stickereACategory
        ? await Product.find({ category: stickereACategory._id }, null, { sort: { '_id': -1 } })
        : [];
