...
export default torbeEco;

export async function getServerSideProps() {
    await mongooseConnect();

    // Find the category with the name "Pixuri Eco"
    const torbeEcoCategory = await Category.findOne({ name: "Torbe-Eco" });

    // If the category is found, fetch products with that category
    const products = torbeEcoCategory
        ? await Product.find({ category: torbeEcoCategory._id }, null, { sort: { '_id': -1 } })
        : [];
