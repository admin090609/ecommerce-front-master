...
export default pungiKraft;

export async function getServerSideProps() {
    await mongooseConnect();

    // Find the category with the name "Pixuri Eco"
    const pungiKraftCategory = await Category.findOne({ name: "Pungi din Kraft" });

    // If the category is found, fetch products with that category
    const products = pungiKraftCategory
        ? await Product.find({ category: pungiKraftCategory._id }, null, { sort: { '_id': -1 } })
        : [];
