...
const suport = ({ products }) => {
    return (
        <div className=" flex flex-col items-center">
            <h1 className="text-3xl mb-8">Suport Cărți de Vizită</h1>
...
export async function getServerSideProps() {
    ...
    const suportCategory = await Category.findOne({ name: "Suport Cărți de Vizită" });

    // If the category is found, fetch products with that category
    const products = suportCategory
        ? await Product.find({ category: suportCategory._id }, null, { sort: { '_id': -1 } })
        : [];
