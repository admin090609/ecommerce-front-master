...
const kraft = ({ products }) => {
    return (
        <div className=" flex flex-col items-center">
            <h1 className="text-3xl mb-8">Cărți de Vizită pe hârtie din kraft</h1>
...
export async function getServerSideProps() {
    ...
    const kraftCategory = await Category.findOne({ name: "Cărți de Vizită pe hârtie din kraft" });

    // If the category is found, fetch products with that category
    const products = kraftCategory
        ? await Product.find({ category: kraftCategory._id }, null, { sort: { '_id': -1 } })
        : [];
