...
const caniCuLogo = ({ products }) => {
    return (
        <div className=" flex flex-col items-center">
            <h1 className="text-3xl mb-8">Căni cu logo</h1>
...
export default caniCuLogo;

export async function getServerSideProps() {
    await mongooseConnect();

    // Find the category with the name "Pixuri Eco"
    const caniCuLogoCategory = await Category.findOne({ name: "Căni cu logo" });

    // If the category is found, fetch products with that category
    const products = caniCuLogoCategory
        ? await Product.find({ category: caniCuLogoCategory._id }, null, { sort: { '_id': -1 } })
        : [];
