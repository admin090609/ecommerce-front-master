...
const usb = ({ products }) => {
  return (
    <div className=" flex flex-col items-center">
      <h1 className="text-3xl mb-8">USB Flash Drive</h1>
...
export async function getServerSideProps() {
  ...
  const usbCategory = await Category.findOne({ name: "USB Flash Drive" });

  // If the category is found, fetch products with that category
  const products = usbCategory
    ? await Product.find({ category: usbCategory._id }, null, { sort: { '_id': -1 } })
    : [];
