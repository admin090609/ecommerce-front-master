...
const rucsac = ({ products }) => {
  return (
    <div className=" flex flex-col items-center">
      <h1 className="text-3xl mb-8">Genți și călătorii</h1>
...
export default rucsac;

export async function getServerSideProps() {
  await mongooseConnect();

  // Find the category with the name "Pixuri Eco"
  const rucsacCategory = await Category.findOne({ name: "Genți și călătorii" });

  // If the category is found, fetch products with that category
  const products = rucsacCategory
    ? await Product.find({ category: rucsacCategory._id }, null, { sort: { '_id': -1 } })
    : [];
