...
export default PixuriEco;

export async function getServerSideProps() {
  await mongooseConnect();

  // Find the category with the name "Pixuri Eco"
  const pixuriEcoCategory = await Category.findOne({ name: "Pixuri Eco" });

  // If the category is found, fetch products with that category
  const products = pixuriEcoCategory
    ? await Product.find({ category: pixuriEcoCategory._id }, null, { sort: { '_id': -1 } })
    : [];
