...
const ecusoane = ({ products }) => {
    return (
        <div className=" flex flex-col items-center">
            <h1 className="text-3xl mb-8">Ecusoane și accesorii</h1>
...
export default ecusoane;

export async function getServerSideProps() {
    await mongooseConnect();

    // Find the category with the name "Pixuri Eco"
    const ecusoaneCategory = await Category.findOne({ name: "Ecusoane și accesorii" });

    // If the category is found, fetch products with that category
    const products = ecusoaneCategory
        ? await Product.find({ category: ecusoaneCategory._id }, null, { sort: { '_id': -1 } })
        : [];
