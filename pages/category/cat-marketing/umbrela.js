...
const umbrele = ({ products }) => {
  return (
    <div className=" flex flex-col items-center">
      <h1 className="text-3xl mb-8">Umbrele</h1>
...
export async function getServerSideProps() {
  ...
  const umbreleCategory = await Category.findOne({ name: "Umbrele" });

  // If the category is found, fetch products with that category
  const products = umbreleCategory
    ? await Product.find({ category: umbreleCategory._id }, null, { sort: { '_id': -1 } })
    : [];
