...
const placute = ({ products }) => {
    return (
        <div className=" flex flex-col items-center">
            <h1 className="text-3xl mb-8">Plăcuțe personalizate</h1>
...
export default placute;

export async function getServerSideProps() {
    await mongooseConnect();

    // Find the category with the name "Pixuri Eco"
    const placuteCategory = await Category.findOne({ name: "Plăcuțe personalizate" });

    // If the category is found, fetch products with that category
    const products = placuteCategory
        ? await Product.find({ category: placuteCategory._id }, null, { sort: { '_id': -1 } })
        : [];
