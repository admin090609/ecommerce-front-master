...
const rame = ({ products }) => {
    return (
        <div className=" flex flex-col items-center">
            <h1 className="text-3xl mb-8">Rame-click</h1>
...
export async function getServerSideProps() {
    ...
    const rameCategory = await Category.findOne({ name: "Rame-click" });

    // If the category is found, fetch products with that category
    const products = rameCategory
        ? await Product.find({ category: rameCategory._id }, null, { sort: { '_id': -1 } })
        : [];
